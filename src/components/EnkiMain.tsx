...
async function getGitHubName (octokit: Octokit) {
    try {
        ...
        const { html_url, login} = res.data;

        const username = html_url.split('/').at(-1) ?? login;
        return username;
    } catch {
        return "";
    }
...
export default function EnkiMain (props: Props) {
    const { octokit: octokitContext } = useContext(props.OctokitContext);
    ...
    async function handleCommitClick(evt: any) {
      ...
      const commit = commits.find((commit) => commit.commit.sha === ref)?.data;
      const files = commit
        /* eslint-disable-next-line */
        /* @ts-ignore */
      ...
      /* @ts-ignore */
      const file =
        displayFiles.find((file: any) => file.name.endsWith('.py')) ?? displayFiles[0];
